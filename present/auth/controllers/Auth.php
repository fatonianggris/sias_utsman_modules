...
class Auth extends MX_Controller {
    ...
    public function index() {
        if ($this->session->userdata('sias-present') == TRUE) {
            redirect('present/dashboard');
        }
        $data['title'] = 'Login Absensi Pegawai | Sekolah Utsman';
        $data['page'] = $this->AuthModel->get_page();
        $this->load->view('present_auth_login', $data);
    }
    ...
    public function login() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $cek_user = $this->AuthModel->check_employe($data);
            $data_user = $this->AuthModel->data_employe($data);
            ...
            if ($cek_user) {
                ...
                if (password_verify(paramEncrypt($data['password']), $cek_user[0]->password)) {
                    ...
                    if ($this->googleCaptachStore($recaptchaResponse, $userIp) == 1) {
                        ...
                        $this->session->set_userdata('sias-present', $data_user);
                        redirect('present/dashboard');
                    } else {
    ...
    public function logout() {
        ...
        $this->session->unset_userdata('sias-present');
