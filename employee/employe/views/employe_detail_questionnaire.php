                                                        ...
                                                        <div class="dropdown dropdown-inline">
                                                            ...
                                                            <div class="dropdown-menu dropdown-menu-sm dropdown-menu-right">
                                                                <ul class="navi flex-column navi-hover py-2">
                                                                    ...
                                                                    <?php if ($user[0]->level_jabatan == 0 && $value->id_jabatan == 1 || $value->id_jabatan == 6 || $value->id_jabatan == 11) { ?>
                                                                    <?php } else { ?>
                                                                        <?php if ($value->status_penilaian_personal == 0 && $value->status_penilaian_sejawat == 0 && $value->status_penilaian_atasan == 0) { ?>
                                                                            <li class="navi-item">
                                                                                <a href="#" data-id_pegawai="<?php echo paramEncrypt($value->id_pegawai); ?>" data-id_penilaian="<?php echo paramEncrypt($value->id_penilaian); ?>" data-id_kuisioner="<?php echo paramEncrypt($questionnaire[0]->id_kuisioner); ?>" data-nama_pegawai="<?php echo strtoupper($value->nama_lengkap); ?>" data-level_tingkat="<?php echo strtoupper($value->level_tingkat); ?>"  data-toggle="modal" data-target="#modal_choose_penilai" class="navi-link" >
                                                                                    <span class="navi-icon"><i class="fas fa-user text-warning"></i></span>
                                                                                    <span class="navi-text text-warning">Pilih Penilai</span>
                                                                                </a>
                                                                            </li>
                                                                        <?php } else { ?>
                                                                            <li class="navi-item">
                                                                                <a href="javascript:change_null('<?php echo strtoupper($value->nip); ?>', '<?php echo strtoupper($value->nama_lengkap); ?>')" class="navi-link" >
                                                                                    <span class="navi-icon"><i class="fas fa-user text-danger"></i></span>
                                                                                    <span class="navi-text text-danger">Pilih Penilai</span>
                                                                                </a>
                                                                            </li>
                                                                        <?php } ?>
                                                                    <?php } ?>
...
<div class="modal fade" id="modal_choose_penilai" tabindex="-1" aria-labelledby="exampleModalSizeLg" aria-hidden="true" role="dialog">
    <div class="modal-dialog modal-dialog-centered modal-md" role="document">
        <div class="modal-content">
            ...
            <form class="form" method="POST" action="<?php echo site_url('employee/employe/report/choose_penilai_employe'); ?>" id="kt_form_tingkat">
                <input type="hidden" class="txt_csrfname" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>"/>
                <input type="hidden" class="hidden" name="id_penilaian" id="id_penilaian" value=""/>
                <input type="hidden" class="hidden" name="id_kuisioner" id="id_kuisioner" value=""/>
                <input type="hidden" class="hidden" name="nama_pegawai" id="nama_pegawai" value=""/>
                <div class="modal-body">
                    <div class="row">
                        <div class="col-xl-1">
                        </div>
                        <div class="col-xl-10">
                            <div class="form-group">
                                ...
                                <select name="id_penilai" id="pilih_penilai" class="form-control form-control-lg" required="">
                                    <option value="">Pilih Penilai</option>
                                </select> 
                                <span class="form-text text-dark"><b class="text-danger">*WAJIB DIISI, </b>pilih Penilai yang ingin ditujukan</span>               
                            </div>
                        </div>
                        <div class="col-xl-1">
                        </div>
...
<script>
    ...
    $('#modal_choose_penilai').on('show.bs.modal', function (e) {
        loadedOptions = false;

        var id_kuisioner = $(e.relatedTarget).data('id_kuisioner');
        var id_penilaian = $(e.relatedTarget).data('id_penilaian');
        var id_pegawai = $(e.relatedTarget).data('id_pegawai')
        var nama_pegawai = $(e.relatedTarget).data('nama_pegawai');
        var level_tingkat = $(e.relatedTarget).data('level_tingkat');

        $(e.currentTarget).find('#title').text('Pilih Penilai untuk Pegawai "' + nama_pegawai + '"');
        $(e.currentTarget).find('input[name="id_penilaian"]').val(id_penilaian);
        $(e.currentTarget).find('input[name="id_kuisioner"]').val(id_kuisioner);
        $(e.currentTarget).find('input[name="nama_pegawai"]').val(nama_pegawai);
        $(e.currentTarget).find('select[name="id_penilai"]').attr('onClick', "choose_penilai('" + id_kuisioner + "','" + level_tingkat + "','" + id_pegawai + "')").prop('selectedIndex', 0);
    });

    function choose_penilai(id_kuisioner, level_tingkat, id_pegawai) {
        if (!loadedOptions) {
            var url = "<?php echo site_url('employee/employe/report/add_ajax_penilai/'); ?>" + id_kuisioner + "/" + level_tingkat + "/" + id_pegawai;
            $('#pilih_penilai').load(url, function () {
...
<script>
    function change_null(nis, name) {
        Swal.fire({
            text: "Mohon Maaf, Data Penilaian Pegawai " + name + " (" + nis + ")  telah dilakukan pengisian oleh Pegawai Lainnya.",
            icon: "error",
