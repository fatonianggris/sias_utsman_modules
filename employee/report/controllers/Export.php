...
class Export extends MX_Controller
{
	...
	{
		$check_qr = $this->ExportModel->get_present_config();

		if ($check_qr[0]->foto_qrcode == '' or $check_qr[0]->foto_qrcode == NULL or $check_qr[0]->foto_qrcode == FALSE) {

			$this->session->set_flashdata('flash_message', err_msg('Maaf, QR Code Belum di Generate!'));
			redirect('employee/configuration/settings/present_configuration');
		} else {

			$data['page'] = $this->ExportModel->get_page();
			$data['qrcode'] = $this->ExportModel->get_present_config();

			if ($data['qrcode'] == NULL or $data['qrcode'][0]->foto_qrcode == '' or $data['qrcode'][0]->foto_qrcode == NULL) {

				$this->session->set_flashdata('flash_message', err_msg('Mohon Maaf!, QR Code Belum di Generate'));
				redirect('employee/configuration/settings/present_configuration');
			} else {

				$html = $this->load->view('pdf_template/qr_print', $data, true);
				$this->pdfgenerator->generate($html, $data['qrcode'][0]->foto_qrcode . '_qr_barcode', 0, '', TRUE, 'A4', 'potrait');
			}
		}
	}
